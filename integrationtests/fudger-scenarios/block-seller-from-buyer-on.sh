...
# Fudger Scenario: Block Seller <- Buyer (Incoming)
# This script starts the tmux test environment and then blocks
# incoming connections to the seller's port (1354)
...
echo "🔴 FUDGER: Block Seller <- Buyer (Incoming)"
echo "==========================================="
echo "This will block incoming connections to port 1354 on the loopback interface"
echo ""
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    RULE_FILE="/tmp/fudger-block-seller-from-buyer.pf"

    # Create the rule file
    cat > "$RULE_FILE" << 'EOF'
# Block incoming traffic to port 1354 on loopback
block drop in quick on lo0 proto {tcp, udp} from any to any port 1354
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "   - Incoming connections to port 1354 on loopback are BLOCKED"
        echo ""
        echo "💡 To turn off: ./block-seller-from-buyer-off.sh"
    else
