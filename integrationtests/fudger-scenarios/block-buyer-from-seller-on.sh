...
# Fudger Scenario: Block Buyer <- Seller (Incoming)
# This script starts the tmux test environment and then blocks
# incoming connections to the buyer's port (1355)
...
echo "🔴 FUDGER: Block Buyer <- Seller (Incoming)"
echo "==========================================="
echo "This will block incoming connections to port 1355 on the loopback interface"
echo ""
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    RULE_FILE="/tmp/fudger-block-buyer-from-seller.pf"

    # Create the rule file
    cat > "$RULE_FILE" << 'EOF'
# Block incoming traffic to port 1355 on loopback
block drop in quick on lo0 proto {tcp, udp} from any to any port 1355
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "   - Incoming connections to port 1355 on loopback are BLOCKED"
        echo ""
        echo "💡 To turn off: ./block-buyer-from-seller-off.sh"
    else
