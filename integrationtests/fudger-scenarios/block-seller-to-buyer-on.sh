...
# Fudger Scenario: Block Seller -> Buyer (Outgoing)
# This script starts the tmux test environment and then blocks
# outgoing connections from the seller to the buyer's port (1355)
...
echo "🔴 FUDGER: Block Seller -> Buyer (Outgoing)"
echo "==========================================="
echo "This will block outgoing connections to port 1355 on the loopback interface"
echo ""
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    RULE_FILE="/tmp/fudger-block-seller-to-buyer.pf"

    # Create the rule file
    cat > "$RULE_FILE" << 'EOF'
# Block outgoing traffic to port 1355 on loopback
block drop out quick on lo0 proto {tcp, udp} from any to any port 1355
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "   - Outgoing connections to port 1355 on loopback are BLOCKED"
        echo ""
        echo "💡 To turn off: ./block-seller-to-buyer-off.sh"
    else
